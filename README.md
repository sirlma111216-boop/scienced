...
|---|---|---|
| 도입 (5) | 자료 하나 읽고 선택 하나(또는 한 줄) | 단계 열기 · 모둠 나누기(나누는 차시) · 응답 n/N ▸ |
| 개념 (15) | 카드 3~4장 읽기 — 문단 셋 + 판단 기준 3줄 | 없음 |
| 활동 (25) | ① 상황+과제 ② 칸 1~2개 ③ 의견 광장(읽기만) ④ 모둠 데이터 ⑤ 게임 | 자료 공개 · 응답 n/N ▸ · 올라온 글 n ▸ · 모둠별 ▸ · 게임 시작 |
| 정리 (5) | 「내 결정의 근거가 된 개념은 무엇이었고, 없었다면 무엇을 달리 했겠는가」 한 칸 + 기준 다시 보기 + 더 읽기 | 응답 n/N ▸ |
...
| `verify:teach` | 등록표 ↔ 본문 · 강사와 학생이 같은 부품 · 수업 중 단추 여섯 이외 없음 · 발표 모드에서 실명 없음 · `/live` 없음 |
...
수업 중 누르는 것은 여섯 가지뿐이다: `단계 열기` · `자료 공개` · `모둠 나누기` · `게임 시작` · `발표 모드` · `응답 펼치기`. `verify:teach` 가 그 밖의 단추를 막는다.

**발표 모드**는 같은 화면의 상태다 — 글자를 키우고 실명을 닉네임으로 바꾼다. 실명(`rosterName`)은 `classes/{cid}/roster` 에만 있고 강사만 읽는다.
