...
1. **한 번 쓴 것은 다시 쓰지 않는다.** 차시마다 쓰는 칸은 넷(교육론 40분 차시는 셋)이고, 제출하면 잠긴다. 지우지 않는다.
...
교수법 한 차시는 **도입 → 개념 → 활동 1 → 활동 2 → 정리** 다섯이다 (강의자 지시 2026-09-22 — 3시간에 두 차시). 교육론 80분 차시는 개념·활동이 두 벌, 40분 차시는 넷. 단계 id 는 고정이다:
`intro · concepts · (activity-1) · activity · (concepts-2 · activity-2) · wrapup`. 교수법의 새 활동 1 은 id 가 `activity-1` 이고 원래 활동은 id `activity` 그대로 「활동 2」로 불린다 — 이미 낸 응답을 옮기지 않으려는 것이다. 차시 파일은 단계를 쓰지 않는다 — `buildSteps(lesson)` 가 `layout` 에서 만든다.

**활동 1**은 비교적 답이 있는 문제다 — 질문하고, 고르거나 쓰고, 서로의 답을 보는 데까지 (모둠·게임 없음). **활동 2**는 정답이 없는 토의 문제다 — 다섯 단 전부.
...
**출석**은 그 질문이다. 매 차시 첫 화면에 「오늘의 질문」이 뜨고, 답한 사람이 그날 출석이다 (`src/lib/attendance.ts`). 모둠 배정 · 응답 n/N 의 N · 게임 참가 · 발표자 뽑기가 모두 그 사람들을 기준으로 돈다. 못 누른 사람은 강사가 명단 서랍에서 넣고, 그 손질만 세션 문서에 남는다. 학기 출석부는 수강생 명단의 「출석」 열이다 (강의자 지시 2026-09-22).
...
| `verify:flow` | 골격 5/6/4 단계(교수법은 활동 1 · 활동 2) · 쓰기 칸 4/4/3 · 활동 다섯 단과 순서(활동 1 은 공유까지) · 도입에 광장 없음 · 정리 문항 꼴 |
...
수업 화면은 **학생 화면 그대로**다. 블록마다 ① 학생이 지금 보는 것(단계를 열기 전엔 같은 「잠김」 카드, 열렸으면 그 칸과 문항) → ② 그 옆의 단추 → ③ 접힌 학생 답 「응답 12/28 ▸」 순서로 그린다. 오늘의 질문·모둠 배너·차시 머리까지 같은 부품(`LessonBody` · `LessonHeader`)이라 강사가 학생에게 무엇이 보이는지 물어볼 일이 없다 (강의자 지시 2026-09-22).

수업 중 누르는 것은 다섯 가지뿐이다: `단계 열기` · `자료 공개` · `모둠 나누기` · `게임 시작` · `응답 펼치기`. `verify:teach` 가 그 밖의 단추를 막는다.

머리의 **명단 · 출석 n/N** 을 누르면 명단 서랍이 열린다 — 오늘 온 사람과 이 단계의 제출이 한 줄에 있고, 체크 상자로 출석을 고친다.
