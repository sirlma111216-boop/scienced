...
강사의 수업 화면(`/teach/:classId/:lessonId`)은 학생 화면과 **같은 블록**을 같은 순서로 그린다. 블록마다 ① 학생이 지금 보는 것 그대로(잠김 카드 · 칸 · 문항, 고를 수 없게) → ② 알약 단추 하나(`단계 열기` · `자료 공개` · `모둠 나누기` · `게임 시작`) → ③ 접힌 응답(「응답 12/28 ▸」) 순서다. 강사 쪽 본문을 집계로 바꿔 끼우지 않는다 — 그러면 강사가 학생이 무엇을 보는지 모른다 (강의자 지시 2026-09-22). 그 밖의 단추를 두지 않는다.

- **출석**은 첫 화면의 「오늘의 질문」이다. 학생이 답을 고르면 그날 출석이고, 수업 화면 머리에 「명단 · 출석 n/N」으로 보인다. 응답 n/N 의 N, 게임 참가, 발표자 뽑기 후보가 모두 그 사람들이다. 서랍 안의 체크 상자로 못 누른 사람을 넣는다 — 수업 흐름 단추를 늘리지 않는다 (강의자 지시 2026-09-22).
- **실명 가리기**는 수업 화면 머리의 체크 상자다. 켜면 모든 이름이 닉네임으로 바뀐다 — 프로젝터에 실명이 나가지 않는 것이 규칙이다. 글자 크기를 바꾸는 발표 모드는 두지 않는다 (강의자 지시 2026-09-18).
- **개념 카드**는 화면 한 장이다. 넘길 층이 없다. 색 블록(lime · lilac · cream · mint) 안에 문단 셋 → 기준 3줄(검은 테두리 상자) → 헷갈리는 것 → 잠깐 확인(같은 검은 테두리 상자, 보기 넷은 테두리 1px 알약 줄, 고르면 2px) → 접힌 「더 읽기」. 잠깐 확인은 답하면 잠기고 정답 보기에 채운 배지 「정답」, 내가 고른 오답에 빈 배지 「내 답」을 붙인다 — 색만으로 가르지 않는다. 강사 화면은 정답과 분포를 「응답 n/N ▸」 접기 안에 둔다.
- 단계 알약과 단계 머리에 시간을 쓰지 않는다 (강의자 지시 2026-09-18).
