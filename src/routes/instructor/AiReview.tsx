...
export function InstructorAiReview() {
  ...
  const clusterOptions = lessonIndex(courseId).flatMap((l) => stepIdsOf(l.layout).filter((s) => !isConceptStepId(s)).map((s) => ({ key: `${l.id}/${s}`, label: `${Number(l.id)}강 ${l.title} · ${stepLabel(l.layout, s)}` })))
  useEffect(() => {
