  ...
  entries: [
    {
      ...
      bridgeToPlain:
        '이 카드의 「먼저 쉽게」에서 「학생이 수업 전에 이미 가지고 있는, 세상을 설명하는 규칙」이라고 한 것이 Ausubel 의 「이미 알고 있는 것」이다. ' +
        '「꼭 알아야 할 것」의 「교사가 먼저 할 일은 정정이 아니라 드러내기」가 「그것을 확인하라」의 교실판이다.',
      limits:
    ...
    },
    {
      ...
      bridgeToPlain:
        '이 카드 전체가 이 주장이다. 「먼저 쉽게」의 「‘틀린 생각’이라고 부르는 대신, 지금 학생이 쓰고 있는 설명이라고 부르는 방식」이 대안적 개념틀이라는 용어 선택 그 자체다. ' +
        '이 강의가 「오개념」이라는 말을 활동 이름(오개념 분기 면담)에서만 쓰고 개념 설명에서는 피하는 이유가 여기 있다.',
      limits:
    ...
    },
    {
      ...
      bridgeToPlain:
        '이 카드의 「왜 필요한가」에서 「‘먼저 써서’, ‘없어져서’ 같은 말이 사고모형을 가리키는 표지」라고 한 것이 이 연구의 방법이다 — 학생의 낱말 그대로를 자료로 삼는다. ' +
        '「헷갈리지 말자」의 「교사가 정리해 준 문장과 다르다 — 정리하는 순간 단서가 지워진다」도 같은 자리에서 왔다.',
      limits:
    ...
    },
    {
      ...
      bridgeToPlain:
        '이 카드의 「정확한 정의」에서 「학생 내부에서는 일관되게 작동하는 설명 체계」라고 한 것이 틀 이론 쪽 입장이다. ' +
        '「전류가 소모된다」가 여러 현상을 한꺼번에 설명한다고 한 「더 읽기」의 목록이, 틀이 종합 모형을 만드는 방식의 예다.',
      limits:
    ...
    },
    {
      ...
      bridgeToPlain:
        '이 카드의 「꼭 알아야 할 것」 다섯째 줄 — 「p-prims — 아주 작은 직관 조각. 상황에 따라 켜졌다 꺼진다」 — 와 「더 읽기」의 「p-prims — 설명보다 작은 조각」이 이 이론이다. ' +
        '「여름엔 지구가 태양에 가깝다」를 「가까울수록 세다」가 잘못 켜진 것으로 읽은 것이 diSessa 식 진단이다.',
      limits:
    ...
    },
    {
      ...
      bridgeToPlain:
        '이 카드의 「더 읽기」에서 「‘써서’는 전류를 물질로 보고 있다 … Chi 는 이것을 존재론적 범주의 착오라고 불렀다」고 한 자리다. ' +
    ...
    },
    {
      ...
      bridgeToPlain:
        '이 카드의 「꼭 알아야 할 것」 둘째 줄 — 「2단계 문항 — 1단계에서 답을 고르고, 2단계에서 그 이유를 고른다」 — 와 「더 읽기」의 「2단계 문항은 이렇게 생겼다」가 이 도구다. ' +
        '이 앱의 형성평가가 답과 이유를 함께 받는 것이 이 형식이고, 오늘 형성평가의 학생 A·B 답안이 그 예다.',
      limits:
