...
지켜야 하는 꼴 (검증기가 센다):
- **교수법은 도입 · 개념 · 활동 1 · 활동 2 · 정리** (강의자 지시 2026-09-22 — 3시간에 두 차시). 활동 1(`activity1` · 단계 id `activity-1`)은 비교적 답이 있는 문제로 질문 · 쓰기 · 공유까지만이고 모둠·게임이 없다. 활동 2(`activity` · 단계 id 는 그대로)는 정답이 없는 토의 문제다. `verify:flow` 가 둘의 꼴을 가른다.
- 교수법의 잠깐 확인은 **답이 하나뿐인 물음**이다 — 「가장 알맞은」 금지 (`verify:concepts`). 물리 소재는 줄인다 — 새 장면은 생물·화학·지구과학에서 먼저.
...
- **강사는 학생이 지금 보는 것을 그대로 본다** (강의자 지시 2026-09-22 — 여러 번 지시했는데 화면이 둘로 갈라져 있었다). 블록마다 순서는 ① 학생 화면 그대로(잠김이면 잠김 카드, 열렸으면 그 칸·문항) → ② 단추 → ③ 접힌 학생 답 「… n/N ▸」. 강사 분기가 학생 것을 집계로 **바꿔 끼우지 않는다**. 블록 부품은 `LessonBody` 에만 둔다 — 경로 파일(`Lesson.tsx` · `Teach.tsx`)은 머리(`LessonHeader`)와 본문(`LessonBody`)만 그린다. `verify:teach` 「같은 화면」이 센다.

- 수업 중 누르는 것은 다섯 가지뿐이다: `단계 열기` · `자료 공개` · `모둠 나누기` · `게임 시작` · `응답 펼치기`. 그 밖의 단추는 `verify:teach` 가 막는다. 「응답 펼치기」는 `<summary>` 접기 요소다. 게임 무대의 「크게 보기」(`components/games/StageZoom.tsx`)는 보기 조작이라 예외다 — 전체 화면이 막히면 고정 판으로 키운다 (강의자 지시 2026-09-22).
...
- **덮개(Overlay·Dialog)의 포커스 이동은 열릴 때 한 번만.** 효과의 의존 목록에 `onClose` 같은 콜백을 두면 부모가 다시 그릴 때마다 포커스를 빼앗아 열어 둔 선택 목록이 닫히고 쓰던 칸에서 커서가 튄다. 최신 콜백은 ref 로 부른다 — `verify:a11y` 「포커스 의존」이 센다 (2026-09-22).
...
따로 배포된 활동 앱(`https://classroom-marble-race.sirlma.workers.dev`)을 iframe 으로 붙인다. 교수법 5강(회전 관문·먼저 2명)·6강(종합 운동장·늦게 2명)도 쓴다 (강의자 지시 2026-09-22 — 연속 배치 예외). 루미 런과 달리 **서버·티켓·webhook 이 없다** (`mode: 'local'`) — 강사 화면(프로젝터) 하나에서 돌고 학생 기기는 참가하지 않는다.
...
- **오늘의 질문에 답한 사람이 오늘 출석이다** (강의자 지시 2026-09-22 · `src/lib/attendance.ts`). 모둠 배정 대상 · 수업 화면의 분모(응답 n/N) · 게임 참가 · 발표자 뽑기 후보가 모두 이 목록 하나다. 수강생 전체를 기준으로 두지 마라 — 결석자가 늘 미제출로 남아 진행이 안 보인다.
- 그래서 질문은 **매 차시** 첫 화면에 뜬다. 나누는 차시에만 「같은 답끼리 모인다」가 붙는다. 질문은 차시 번호 자리에서 고른다(`questionForLessonNumber`) — 과목마다 시작 자리가 어긋나 있다.
- 답을 못 누른 사람은 강사가 명단 서랍(또는 모둠 나누기의 출석 줄)에서 넣는다. **그 손질만** 세션 문서 `attendance: {in, out}` 에 남는다. 출석 목록 자체를 저장하지 않는다.
