...
- **장면·대사는 상황극이 아니다** (강의자 지적 2026-09-22). 도입 자료·활동 상황·「교실에서」를 쓸 때 넷을 스스로 묻는다 — 그 학생이 정말 그렇게 말하는가 · 한 번 읽고 이해되는가 · 개념과 이어지는 논리(무엇을 믿었나 → 봤나 → 말했나)가 문장 안에 있는가 · 과학교사가 실제로 겪는 장면인가. 검증기가 못 세니 `docs/검토/_작성-지침.md` 「장면과 대사」를 읽고 「이렇게 정했다」에 적는다.
...
- **어느 차시가 나누는가는 `src/lib/group-round.ts` 의 `defaultFormationLessons` 하나가 정한다** (강의자 확정 2026-09-22). 교육론은 매 차시. 교수법은 12강까지 홀수 차시(1·3·5·7·9·11)에 나누고 그다음 짝수 차시가 앞 차시를 따르며, **13강부터는 매 차시** 나눈다. 다른 곳에 홀짝 계산을 두지 않는다 — `verify:groups` 의 「전수조사」가 30차시를 하나씩 센다.
- 나누지 않는 차시에는 **질문 블록도 [모둠 나누기] 단추도 없다.** 모둠이 어느 차시 것인지는 모둠 배너가 말한다. 앞 차시 모둠을 쓰는 자리에서 새로 나누면 둘이 뒤섞인다 (강의자 지적 2026-09-22). 단추만 지우는 것으로는 부족하다 — 블록 제목이 「오늘의 질문 · 출석」으로 갈리고, 안내가 「이 차시는 모둠을 나누지 않는다 — 7강에서 나눈 모둠을 그대로 쓴다」처럼 **이은 차시 번호를 댄다** (`followedLesson`). 같은 분홍 블록에 제목만 「오늘의 질문」이면 강사가 그 자리를 또 모둠 자리로 읽는다 (강의자 지적 2026-09-26). `verify:groups` 「나누지 않는 차시」가 센다.
...
- **질문은 모둠을 나누는 차시에만 뜬다** (강의자 지시 2026-09-26). 교수법은 3시간에 두 차시를 잇달아 하므로 7·8강이 같은 날이다 — 앞 차시에서 출석도 받고 모둠도 나눴으니 따르는 차시(2~12 짝수)에는 「오늘의 질문」 블록이 **아예 없다.** 그 차시의 출석·응답 n/N·게임 참가는 앞 차시의 출석을 그대로 읽고, 명단 서랍의 손질도 앞 차시 문서에 적힌다 (`attendanceLessonOf`). 같은 분홍 블록을 출석용으로 또 두면 강사가 그 자리를 모둠 자리로 읽는다.
...
- 질문은 차시 번호 자리에서 고른다(`questionForLessonNumber`) — 과목마다 시작 자리가 어긋나 있다.
