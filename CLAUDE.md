...
- 나누지 않는 차시에는 **[모둠 나누기] 단추가 없다.** 앞 차시 모둠을 쓰는 자리에서 새로 나누면 둘이 뒤섞인다 (강의자 지적 2026-09-22). 단추만 지우는 것으로는 부족하다 — 블록 제목이 「오늘의 질문 · 출석」으로 갈리고, 안내가 「이 차시는 모둠을 나누지 않는다 — 7강에서 나눈 모둠을 그대로 쓴다」처럼 **이은 차시 번호를 댄다** (`followedLesson`). 같은 분홍 블록에 제목만 「오늘의 질문」이면 강사가 그 자리를 또 모둠 자리로 읽는다 (강의자 지적 2026-09-26). `verify:groups` 「나누지 않는 차시」가 센다.
...
- 그래서 질문은 **매 차시** 첫 화면에 뜬다. 나누는 차시에만 「같은 답끼리 모인다」와 [모둠 나누기]가 붙는다. 질문은 차시 번호 자리에서 고른다(`questionForLessonNumber`) — 과목마다 시작 자리가 어긋나 있다.
